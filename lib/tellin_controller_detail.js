...
DemoFactory.prototype={
	_createItem:function(str,reg,name){
		var _reg = new RegExp(reg.source,'g'),
			_result = _reg.exec(str);
		if(_result.length>3){
	...
	renderHTML:function(str) {
		...
		return str.replace(RegDemoCon,function(word) {
			var _reg = new RegExp(RegDemoCon.source,'g');
			_id= _reg.exec(word)[1];
			_item = me.Demo[_id];
			if(_item!=null){
...
ControllerDetail.prototype={
	...
	_renderFile:function(rootPath,node,data,config){
		...
		var output = core.mergeAndFormatPath(rootPath,data.NameHTML),
			bread= core.getBreadCrumbs(output,node._deep);
		...
		//console.log(core.mergeAndFormatPath(rootPath,data.NameHTML));

		fs.writeFile(output,_text,{
			encoding:'utf8',
			mode:0o666,
			flag:'w'
		},function(err){
